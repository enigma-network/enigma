...
function generateCompose(modelIds: string[], serverUrl: string, hasNvidia: boolean, backend = 'ollama'): string {
  const lines: string[] = []
  const volumes: string[] = []
  const seen: Record<string, number> = {}
  const useOllama = backend === 'ollama'
  const backendPort = BACKEND_PORTS[backend] ?? 11434
  ...
  for (const modelId of modelIds) {
    ...
    if (useOllama) {
      ...
      // External backend already running on host — node connects via host.docker.internal
      lines.push(`  ${nodeService}:`)
      lines.push(`    image: ghcr.io/enigma-network/enigma-node:latest`)
      lines.push(`    command:`)
      lines.push(`      - "-server"`)
...
export async function GET(req: Request) {
  ...
  if (os === 'windows') {
    return windowsScript({ modelIds, ollamaServices, serverUrl, gpu, hasNvidia, backend })
  }
  if (os === 'mac') {
    return macScript({ modelIds, ollamaServices, serverUrl, gpu, backend })
  }
  return linuxScript({ modelIds, ollamaServices, serverUrl, gpu, hasNvidia, backend })
}

// ─── Linux ────────────────────────────────────────────────────────────────────

function linuxScript({ modelIds, ollamaServices, serverUrl, gpu, hasNvidia, backend }: {
  modelIds: string[], ollamaServices: string[], serverUrl: string, gpu: string, hasNvidia: boolean, backend: string
}) {
  const compose = generateCompose(modelIds, serverUrl, hasNvidia, backend)
  const useOllama = backend === 'ollama'
...
function macScript({ modelIds, ollamaServices, serverUrl, gpu, backend }: {
  modelIds: string[], ollamaServices: string[], serverUrl: string, gpu: string, backend: string
}) {
  const compose = generateCompose(modelIds, serverUrl, false, backend)
  const useOllama = backend === 'ollama'
  const pullCmds = ollamaServices.map((svc, i) =>
    `echo "Lade Modell: ${modelIds[i]}..."\ndocker compose exec -T ${svc} ollama pull ${modelIds[i]} || echo "Warnung: konnte Modell nicht laden"`
  ).join('\n')
...
function windowsScript({ modelIds, ollamaServices, serverUrl, gpu, hasNvidia, backend }: {
  modelIds: string[], ollamaServices: string[], serverUrl: string, gpu: string, hasNvidia: boolean, backend: string
}) {
  const compose = generateCompose(modelIds, serverUrl, hasNvidia, backend)
  const useOllama = backend === 'ollama'
